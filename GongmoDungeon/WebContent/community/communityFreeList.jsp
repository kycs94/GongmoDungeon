<%@page import="team04.project.community.Community_writeDTO"%>
...
<%
	...
	if(sel!=null&&search!=null){//검색함
		count = dao.getFreeSearchCount(sel, search);
		if(count>0){
			articleList = dao.getFreeSearchList(startRow, endRow, sel, search); 
		}
	}else{//검색 안함
		// 전체 글의 개수 가져오기
		count = dao.getFreeAllCount(); // DB에 저장되어있는 전체 글의 개수를 담아놓을 변수
		
		// 글이 하나라도 있으면 글들을 다시 가져오기
		if(count > 0){
			articleList = dao.getFreeAllList(startRow, endRow);
		}
	}
			...
			<div class="commu">
				...
				<div class="commuMain">
					<div class="commuHead" style="text-align:right;">
						...
						<button onclick="window.location='communityWriteForm.jsp'">글쓰기</button>
						<%} %>
					</div>
					<div class="commuBody">
							...
							<%if(count>0){for(int i=0; i<articleList.size(); i++){
								...
								<tr>
									...
									<td>
										...
										<%= article.getCMBoard_title() %></a>
										<%if(article.getCMBoard_commCount()>0){ %>
										[<%= article.getCMBoard_commCount() %>]
										<%} %>
									</td>
									<td>
										<a href="../msg/sendmsgForm.jsp?send=<%=sender%>&receive=<%= article.getCMBoard_id() %>"
										onclick="window.open(this.href, '메시지 보내기', 'width=500px,height=500px,toolbars=no,scrollbars=no'); return false;"
										><%= article.getCMBoard_id() %></a>
									</td>
							...
							if(count>0){
								...
								if(sel!=null&&search!=null){ // 검색 했을때
									...
									if(startPage > pageBlock){%>
										<a href="communityFreeList.jsp?pageNum=<%=startPage - pageBlock%>&sel=<%=sel%>&search=<%=search%>">&lt;</a>
										
									<%}
									
									// 페이지 번호 뿌리기
									for(int i = startPage; i <= endPage; i++){%>
										<a href="communityFreeList.jsp?pageNum=<%=i%>&sel=<%=sel%>&search=<%=search%>" 
									...
									<a href="communityFreeList.jsp?pageNum=<%=startPage + pageBlock%>&sel=<%=sel%>&search=<%=search%>" class="pageNums">&gt;</a>
									<%}
								}else{ // 검색 안했을때
									//왼쪽 꺽쇄 : startPage 가 pageBlock(10) 보다 크면 보여줌
									if(startPage > pageBlock){%>
										<a href="communityFreeList.jsp?pageNum=<%=startPage - pageBlock%>" >&lt;</a>
										
									<%}
									
									// 페이지 번호 뿌리기
									for(int i = startPage; i <= endPage; i++){%>
										<a href="communityFreeList.jsp?pageNum=<%=i%>" 
											<% if(i==Integer.parseInt(pageNum)){%>class="active"<%} %>
										>&nbsp;<%=i%>&nbsp;</a>
									<%}
									...
									//오른쪽 꺽쇄 : 전체 페이지 개수가 현재 보이는 페잊에서 마지막 번호보다 크면 보여줌
									if(endPage < pageCount){%>
									<a href="communityList.jsp?pageNum=<%=startPage + pageBlock%>">&gt;</a>
									<%}
								}
							}
						%>
					...
					<div class="commuFoot"  style="text-align:center;">
						<form action="communityFreeList.jsp">
							<select name="sel">
								<option value="CMBoard_title" 
									<% if(sel!=null&&sel.equals("CMBoard_title")){ %>
										selected
									<%} %>
								>제목</option>
								<option value="CMBoard_id"
									<% if(sel!=null&&sel.equals("CMBoard_id")){ %>
										selected
									<%} %>
								>글쓴이</option>
							</select>
